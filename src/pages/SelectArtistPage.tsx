...
export default function SelectArtistPage() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDark ? "dark-vars bg-[#0A0A0F]" : "bg-white"}`}>
      {/* 헤더 — 여백 넓게 */}
      <div className="px-6 pt-16 pb-6 animate-fade-in-down">
        <div className="flex justify-center mb-4">
          <span className="text-3xl">✦</span>
        </div>
        <h1 className={`text-[26px] font-black text-center leading-tight ${isDark ? "text-white" : "text-gray-900"}`}>
          Who do you stan?
        </h1>
        <p className={`text-sm text-center mt-3 leading-relaxed ${isDark ? "text-gray-400" : "text-gray-500"}`}>
          {t("select.subtitle")}
        </p>
      </div>
      ...
      {/* 하단 버튼 */}
      <div className={`fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t ${isDark ? "from-[#0A0A0F] via-[#0A0A0F]/95 to-transparent" : "from-white via-white/95 to-transparent"}`}>
        <button
          disabled={selectedArtists.length === 0}
          onClick={handleContinue}
          className={`w-full py-4 rounded-full text-[15px] font-bold transition-all duration-300 ${
