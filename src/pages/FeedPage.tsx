...
export default function FeedPage() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDark ? "dark-vars bg-[#0A0A0F] text-white" : "bg-gray-50 text-gray-900"}`}>
      {/* 헤더 */}
      <div className={`sticky top-0 z-10 backdrop-blur-md border-b ${isDark ? "bg-[#0A0A0F]/95 border-[#1E1E2E]" : "bg-white/95 border-gray-100"}`}>
        ...
        <div className="px-4 pb-3 flex gap-3 overflow-x-auto scrollbar-hide">
          <button onClick={() => refetch()} className="flex flex-col items-center gap-1.5 flex-shrink-0">
          ...
          {selectedArtistInfo.map((a) => (
            <div key={a.id} className="flex flex-col items-center gap-1.5 flex-shrink-0">
              ...
              <span className={`text-[10px] max-w-[48px] truncate ${isDark ? "text-gray-400" : "text-gray-500"}`}>{a.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 pt-4 pb-2 flex items-center justify-between">
        ...
        <button onClick={() => refetch()} className="text-xs text-[#E91E63] font-semibold">View all</button>
      </div>

      {/* 피드 */}
      <div className="flex-1 overflow-y-auto px-4 pb-24 flex flex-col gap-4 scrollbar-hide">
        {isLoading ? (
